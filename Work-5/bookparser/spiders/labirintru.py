...
class LabirintruSpider(scrapy.Spider):
    ...
    def parse(self, response: HtmlResponse):
        ...
        if next_page:
            ...
            # Рекурсивный вызов функции со ссылкой следующей страницей yield response.follow(next_page, callback=self.parse)
        else:
            self.logger.info('Следующих страниц больше нет')

        # Собираем ссылки со всех книг на странице links = response.xpath("//a[@class='product-card__img']/@href").getall()
        for link in links:
            # Делаем запросы для каждой ссылки, ответы от запросов будут направлятся в book_parse yield response.follow(link, callback=self.book_parse)

    # Функция парсит страницу книги
    def book_parse(self, response: HtmlResponse):
        name = response.xpath("//h1/text()").get().strip()  # название книги author = response.xpath("//a[@data-event-label='author']/text()").getall()  # автор translator = response.xpath("//a[@data-event-label='translator']/text()").getall()  # переводчик
        ...
        year = response.xpath("//div[@class='publisher']/text()").re_first(r'\d{4}')  # год
        ...
        weight = response.xpath("//div[@class='weight']/text()").get()  # вес книги dimensions = response.xpath("//div[@class='dimensions']/text()").get()  # размеры
